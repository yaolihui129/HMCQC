       ...
       // dump($info);
        if(!$info) {// 上传错误提示错误信息
            ...
            if ($db->save($_POST)){
                ...
                $image->thumb(900, 500,\Think\Image::IMAGE_THUMB_CENTER)->save('./Public/Upload/'.$info['himg']['savepath'].'/thumb_'.$info['himg']['savename']);
                $this->success("上传成功！");
            }else{
                $this->error("上传失败！");
            }
    ...
    public function imga(){
        ...
        if(!$info) {// 上传错误提示错误信息
            ...
            if ($db->save($_POST)){
                ...
                $image->thumb(900, 500,\Think\Image::IMAGE_THUMB_CENTER)->save('./Public/Upload/'.$info['aimg']['savepath'].'/thumb_'.$info['aimg']['savename']);
                $this->success("上传成功！");
            }else{
                $this->error("上传失败！");
            }
